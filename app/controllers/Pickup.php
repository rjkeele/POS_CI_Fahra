...
class Pickup extends MY_Controller {
    ...
    function searchPickupCustomer($search = null) {
        if ($this->input->get('search')) {
            $search = $this->input->get('search');
        }
        $customers = $this->site->getLimitPickupCustomers($search);
        echo json_encode($customers);
    }

    function searchPickupByCustomer($id = null) {
        if ($this->input->get('id')) {
            $id = $this->input->get('id');
        }
        $customers = $this->site->getLimitPickupByCustomers($id);
        echo json_encode($customers);
    }
    ...
    function index() {
        ...
        // $this->page_construct('pickup/index', $this->data, $meta);
        $this->load->view($this->theme . 'pickup/index', $this->data, $meta);
    }
    ...
    function get_sales($id = null) {
        ...
        if ($this->db->dbdriver == 'sqlite3') {
            $this->datatables->select("sales.id, cloth_rack.label_id,  sales.customer_name, sales.total, sales.total_tax, sales.total_discount, sales.total_quantity, sales.pickup_date, sales.grand_total,  sales.paid, sales.status, sales.pick")
            ->join('cloth_rack', 'sales.id=cloth_rack.ticket_id');
            // $this->datatables->select("id, strftime('%Y-%m-%d %H:%M', date) as date, customer_name, total, total_tax, total_discount, total_quantity, pickup_date, grand_total, paid, status, pick");
        } else {
            // $this->datatables->select("id, DATE_FORMAT(date, '%Y-%m-%d %H:%i') as date, customer_name, total, total_tax, total_discount, total_quantity, pickup_date, grand_total, paid, status, pick");
            $this->datatables->select("sales.id, cloth_rack.label_id,  sales.customer_name, sales.total, sales.total_tax, sales.total_discount, sales.total_quantity, sales.pickup_date, sales.grand_total,  sales.paid, sales.status, sales.pick")
            ->join('cloth_rack', 'sales.id=cloth_rack.ticket_id');     
        }
    ...
    function setPick() {
        ...
        $this->sales_model->picked( $id, $pick);
