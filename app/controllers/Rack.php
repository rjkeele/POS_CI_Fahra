...
class Rack extends MY_Controller {
    ...
    public function index($sid = null, $eid = null) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_id) && isset($real_unit_price) && isset($item_quantity)) {
                    ...
                    $subtotal = $this->tec->formatDecimal((($item_net_price * $item_quantity) + $pr_item_tax), 4) + $item_upcharge_cost + $item_color_cost;
                    ...
                    $total += $this->tec->formatDecimal(($item_net_price * $item_quantity), 4) + $item_upcharge_cost + $item_color_cost;
                }
            }
            if (empty($products)) {
        ...
        if ($this->form_validation->run() == true && !empty($products)) {
            ...
            $this->load->view($this->theme . 'rack/index', $this->data, $meta);
        }
    }

    public function searchPickupCustomer($search = null) {
        if ($this->input->get('search')) {
            $search = $this->input->get('search');
        }
        $customers = $this->site->getLimitTagCustomers($search);
    ...
    public function get_product($code = null) {
        ...
        if ($product = $this->pos_model->getProductByCode($code)) {
            ...
            $product->unit_price = $product->unit_price + $upchrges_total_cost + $color_total_cost;
    ...
    public function view($sale_id = null, $noprint = null) {
        ...
        $this->data['barcode'] = $this->invoice_barcode($this->data['invoice_id'], 'code128', 60);
        $rows = $this->data['rows'];
        $tags = array();
        $tag_num = 1;
        foreach ($rows as $row) {
            for ($i = 0; $i < intval($row->quantity)*intval($row->set_pc); $i++){
                $tags[] = array(
                    'barcode' => $this->invoice_barcode($this->data['invoice_id'].'-'.$tag_num, 'code128', 60),
                    'lgID' => $row->upcharge_name
                );
                $tag_num++;
            }
        }
    ...
    public function p($bo = 'order') {
        ...
        $session_user = $this->input->post('session_user');
        $newRackData = $this->pos_model->insertRackData($rack_number, $ticket_number, $session_user); 
        $customerData = $this->pos_model->getCustomerByIdRack($ticket_number);
        $user_data = $this->settings_model->getSettingsByID($session_user);

        $phone = $user_data->tel;
        $phone_customer = $customerData[0]->phone;
        $auth_id = "MANTDIYMNMNMFHZTM1ZJ";
        $auth_token = "YjZlZDRjNDI1OTRlOTNkZTI1NzZkZTE5Nzc5Y2Y5";
        
        if ($customerData[0]->kind_phone ==="voicephone") {
            
            $client = new RestClient($auth_id,  $auth_token);
            try {
                $response = $client->calls->create(
                    '$phone', //$phone, # The phone number to be used as the caller id
                    [$phone_number],//$phone_number, # The phone numer to which the call will be placed
                    'https://api.plivo.com/v1/Account/MANTDIYMNMNMFHZTM1ZJ/Call/',
                    'POST',
                    [
                        'ring_url' =>' http://9e52b86f.ngrok.io/responce',
                    ],
                    'Ticket ' . $ticket_number . ' is ready for pickup.'
                );
                print_r($response);
            }
        ...
        } else if ($customerData[0]->kind_phone === "cellphone") {
            ...
            $response = $client->messages->create(
                '$phone', //$phone, # The phone number to be used as the caller id
                [$phone_number],//$phone_customer # The phone numer to which the call will be placed
                'Ticket ' . $ticket_number . ' is ready for pickup.',
                ["url"=>"https://api.plivo.com/v1/Account/" . $auth_id . "/Message"]
                 // 'powerpack'//'86627ff5-fdf3-47ee-b850-e7930eb29a08'//
            );
            print_r($response);
            // Prints only the message_uuid
            print_r($response->getmessageUuid(0));
        } else {
            alert('Choose type of phone!');
        }
