...
class Tag extends MY_Controller {
    ...
    public function index($sid = null, $eid = null) {
        ...
        if ($this->form_validation->run() == true && !empty($products)) {
            ...
            $this->load->view($this->theme . 'tag/index', $this->data, $meta);
        }
    }
    ...
    function invoice_barcode($product_code = NULL, $bcs = 'code128', $height = 20) {
    ...
    public function view($sale_id = null, $noprint = null) {
        ...
        $this->data['image'] = $this->sales_model->getTagsById($sale_id);
        ...
        foreach ($rows as $row) {
            for ($i = 0; $i < intval($row->quantity)*intval($row->set_pc); $i++){
                $tags[] = array(
                    'barcode' => $this->invoice_barcode($this->data['invoice_id'].'-'.$tag_num, 'code128', 20),
    ...
    public function checkImage() {
        ...
        $tag_length = $_GET['tag_length'];           
        // echo $image_id;
        $filename = 'pic_'.$image_id.'_'.$tag_length. '.jpg';          
        // echo $filename;       
       
        if( move_uploaded_file($_FILES['webcam']['tmp_name'],'uploads/snaps/'.$filename) ){
            // echo $filename;         
            $this->sales_model->updateSnap($image_id, $filename);
        }  
    }
	...
	public function printDate(){
        ...
        $this->sales_model->updatePrintDate($print_id,  $date_print);
            //  echo $date_print;
     }
