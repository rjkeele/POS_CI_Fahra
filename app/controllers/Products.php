...
class Products extends MY_Controller
{
    ...
    function get_products($store_id) {
        ...
        if ($this->Admin) {
            $this->datatables->select($this->db->dbprefix('products').".id as pid, ".$this->db->dbprefix('products').".image as image, ".$this->db->dbprefix('products').".code as code, ".$this->db->dbprefix('products').".name as pname, ".$this->db->dbprefix('categories').".name as cname, psq.quantity, tax, tax_method, cost, ".$this->db->dbprefix('products').".set_pc, (CASE WHEN psq.price > 0 THEN psq.price ELSE {$this->db->dbprefix('products')}.price END) as price, ".$this->db->dbprefix('products').".press_only, barcode_symbology", FALSE);
        } else {
            $this->datatables->select($this->db->dbprefix('products').".id as pid, ".$this->db->dbprefix('products').".image as image, ".$this->db->dbprefix('products').".code as code, ".$this->db->dbprefix('products').".name as pname, ".$this->db->dbprefix('categories').".name as cname, psq.quantity, tax, tax_method, ".$this->db->dbprefix('products').".set_pc, (CASE WHEN psq.price > 0 THEN psq.price ELSE {$this->db->dbprefix('products')}.price END) as price, ".$this->db->dbprefix('products').".press_only, barcode_symbology", FALSE);
        }
    ...
    function add() {
        ...
        // if ($this->input->post('type') != 'service') {
            ...
            $this->form_validation->set_rules('set_pc', lang("set_pc"), 'required|is_numeric');
            $this->form_validation->set_rules('press_only', lang("press_only"), 'required|is_numeric');
        // }
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array(
                ...
                'set_pc' => $this->input->post('set_pc'),
                'press_only' => $this->input->post('press_only'),
