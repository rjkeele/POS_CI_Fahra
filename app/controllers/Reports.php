...
class Reports extends MY_Controller
{
    ...
    function get_sales() {
        ...
        $this->datatables
        ->select("id, date, customer_name, total, total_tax, total_discount, grand_total, paid, (grand_total-paid) as balance, status, tick_print_date, tag_print_date")
        ->from('sales');
    ...
    function get_tag_count() {
        ...
        $this->load->library('datatables');
        $this->datatables
        ->select("id, date, customer_name, total_quantity, total_items, status, tick_print_date, tag_print_date")
        ->from('sales');
        if ($this->session->userdata('store_id')) {
            $this->datatables->where('store_id', $this->session->userdata('store_id'));
        }
    ...
    function get_tagged() {
        ...
        $this->datatables
        ->select("id, date, customer_name, total_quantity, total_items, status, tick_print_date, tag_print_date")
        ->from('sales')->where('tag_print_date IS TRUE');
        if ($this->session->userdata('store_id')) {
            $this->datatables->where('store_id', $this->session->userdata('store_id'));
        }
        $this->datatables->unset_column('id');
    ...
    function get_notTagged() {
        
        $this->load->library('datatables');
        $this->datatables
        ->select("id, date, customer_name, total_quantity, total_items, status, tick_print_date, tag_print_date")
        ->from('sales')->where('tag_print_date IS NULL');
        if ($this->session->userdata('store_id')) {
            $this->datatables->where('store_id', $this->session->userdata('store_id'));
        }
        $this->datatables->unset_column('id');
    ...
    function get_noTag() {
        ...
        $this->datatables
        ->select("id, date, customer_name, total_quantity, total_items, status, tick_print_date, tag_print_date")
        ->from('sales')->where('tag_print_date IS NULL AND tag_print_date');
        if ($this->session->userdata('store_id')) {
            $this->datatables->where('store_id', $this->session->userdata('store_id'));
        }
        $this->datatables->unset_column('id');
