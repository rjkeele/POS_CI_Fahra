...
class Site extends CI_Model
{
    ...
    public function getLimitCustomers($search = Null) {
    ...
    public function getLimitPickupCustomers($search = Null) {
        $this->db->select('customers.id, customers.name AS text, customers.phone');
        $this->db->from('sales');
        $this->db->join('customers', 'customers.id = sales.customer_id', 'left');
        // $this->db->where(array('sales.id' => 10));
        if ($search) {
            // $this->db->like('sales.id', $search);
    ...
    public function getLimitPickupByCustomers($id = Null) {
        $this->db->select('sales.id AS salesId, customers.id, sales.customer_name AS text, sales.pickup_date, customers.phone');
        $this->db->from('sales');
        $this->db->join('customers', 'customers.id = sales.customer_id');
        if ($id) {
            $this->db->where(array('customers.id' => $id));
        }
        // $this->db->limit(5);
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
    ...
    public function getLimitTagCustomers($search = Null) {
        $this->db->select('sales.id, sales.customer_name AS text, sales.pickup_date, customers.phone');
        $this->db->from('sales');
        $this->db->join('customers', 'customers.id = sales.customer_id');
        // $this->db->where(array('sales.id' => 10));
        if ($search) {
            $this->db->like('sales.id', $search);
            // $this->db->or_like('sales.customer_name', $search);
            // $this->db->or_like('customers.phone', $search);
        }
        $this->db->limit(5);
        $q = $this->db->get();
    ...
    public function getAllUsers() {
        $this->db->select("{$this->db->dbprefix('users')}.id as id, first_name, last_name, {$this->db->dbprefix('users')}.email,security_pin, company, {$this->db->dbprefix('groups')}.name as group, active, {$this->db->dbprefix('stores')}.name as store")
    ...
    public function getAllColors() {
        $this->db->order_by('id');
        $this->db->where_not_in('name', 'Skip');
        $q = $this->db->get('cloth_colors');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getAllUpcharges() {
        $this->db->order_by('id');
        $q = $this->db->get('cloth_upcharges');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getAllSpotlists() {
        $this->db->order_by('id');
        $q = $this->db->get('cloth_spotlists');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getColorByID($id) {
        $q = $this->db->get_where('cloth_colors', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getUpchargeByID($id) {
        $q = $this->db->get_where('cloth_upcharges', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getSpotlistByID($id) {
        $q = $this->db->get_where('cloth_spotlists', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getMaterialByID($id) {
        $q = $this->db->get_where('cloth_materials', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getColorByCode($hash_code) {
        $q = $this->db->get_where('cloth_colors', array('hash_code' => $hash_code), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getUpchargeByName($name) {
        $q = $this->db->get_where('cloth_upcharges', array('name' => $name), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getSpotlistByName($name) {
        $q = $this->db->get_where('cloth_spotlists', array('name' => $name), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getMaterialByName($name) {
        $q = $this->db->get_where('cloth_materials', array('name' => $name), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
