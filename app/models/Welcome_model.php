...
class Welcome_model extends CI_Model
{
    ...
    public function getChartDataTag($user_id = NULL) {
        if(!$this->Admin) {
            $user_id = $this->session->userdata('user_id');
        }
        if ($this->db->dbdriver == 'sqlite3') {

            $this->db->select("strftime('%Y-%m-%d', date) as day, COUNT(*) as tagged")->where("date >= datetime('now','-1 month') AND tag_print_date", NULL, FALSE)
            // ->order_by("strftime('%Y-%m', date)", 'asc')
            ->group_by("strftime('%Y-%m-%d', date)");
        } else {

            $sql = "SELECT t1.DAY, t1.not_tagged, t2.tagged FROM (SELECT DATE_FORMAT(DATE, '%Y-%m-%d') AS DAY, COUNT(*) AS not_tagged FROM tec_sales WHERE DATE >= DATE_SUB( NOW() , INTERVAL 1 MONTH) AND tag_print_date IS NULL GROUP BY DATE_FORMAT(DATE, '%Y-%m-%d')) t1 LEFT JOIN (SELECT DATE_FORMAT(DATE, '%Y-%m-%d') AS DAY, COUNT(*) AS tagged FROM tec_sales WHERE DATE>=DATE_SUB(NOW(), INTERVAL 1 MONTH) AND tag_print_date IS TRUE GROUP BY DATE_FORMAT(DATE, '%Y-%m-%d')) t2 ON (t1.DAY = t2.DAY)";
