...
class Sales_model extends CI_Model
{
    ...
    public function picked($id, $pick) {
        
        if ($this->db->update('sales', array('pick' => $pick), array('id' => $id))) {
            return true;
        }
        return false;
    }
    ...
    public function getSalesById($id) {   
        $q = $this->db->get_where('cloth_rack', array('id' => $id));
        if ($q->num_rows() > 0) {
            if( $q->num_rows() > 0 ) {
                return $q->row();
            }
    ...
    public function updateSnap($snap_id, $snap_name) {
      
        $q = $this->db->get_where('sales', array('id' => $snap_id), 1);
        ...
        if($this->db->update('sales', array('snaps' => $row->snaps . ',' . $snap_name), array('id' => $snap_id))){
	...
	public function updatePrintDate($print_id,  $date_print){

        if($this->db->update('sales', array('print_date' => $date_print, 'tag_print_date' => $date_print), array('id' => $print_id))){
            // echo $date_print;
        }
    }

    public function updatePrintDateTick($print_id,  $date_print){

        if($this->db->update('sales', array('tick_print_date' => $date_print), array('id' => $print_id))){
            // echo $date_print;
        }
    }
