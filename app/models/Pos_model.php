...
class Pos_model extends CI_Model
{
    ...
    public function suspendSale($data, $items, $did = null)
    {
        ...
        if ($did) {
            ...
            // $this->tec->print_arrays($data, $items, $did);
    ...
    {
        $j = "(SELECT id, code, name, tax_method, set_pc from {$this->db->dbprefix('products')}) P";
    ...
    /* myone from */
    public function getUserPin($user_id, $secPin)
    {
        $q = $this->db->get_where('users', array('security_pin' => $secPin)); // 'id' => $user_id,
        if ($q->num_rows() > 0) {
            return $q->result()[0];
        }
        return false;
    }
    public function findCustomer($cname)
    {
        // $this->db->where("`name` LIKE '%$cname%')");
        $this->db->where("`name` LIKE '%$cname%'");
        // $q = $this->db->get_where('customers', array('name' => $cname));
    ...
    public function insertRackData($rack_number, $ticket_number, $session_user)
    {
        $date = date('Y-m-d H:i:s');
        $data = array(
            'location_name' => $rack_number,
            'label_id' => $rack_number,
            'ticket_id' => $ticket_number,
            'user_id' => $session_user,
            'created' => $date
        );        
        $q = $this->db->get_where('cloth_rack', array('ticket_id' => $ticket_number));
        
        if ($q->num_rows() > 0) {
            $this->db->update('cloth_rack', $data, array('ticket_id' => $ticket_number));
            return true;
        }else{
            if($this->db->insert('cloth_rack', $data))
                return $rack_id = $this->db->insert_id();              
        }
    }

    public function getCustomerByIdRack($id){

        $this->db->select('sales.id as ticket, customers.phone as phone, customers.kind_phone as kind_phone');
        $this->db->from('sales');
        $this->db->join('customers', 'customers.name=sales.customer_name');
