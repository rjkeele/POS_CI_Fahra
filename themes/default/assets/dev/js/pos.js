        ...
        if(spositems) {
            $.each(spositems, function() {
                ...
                var product_id = item.row.id,
                    ...
                    item_color_price = item.row.color_total_cost,
                    item_upcharge = item.row.upcharge_name,
                    item_material = item.row.material_name,
                    item_material_price = item.row.material_total_cost,
                // change upcharge price (current only press. etc.....)    myone
                    item_upcharge_price = item.row.upcharge_name == 'Press Only'? 0: item.row.upchrges_total_cost;
                ...
                tr_html =
                    ...
                    '"><input name="material_name[]" type="hidden" value="' +
                    item.row.material_name +
                    '"><input name=material_cost[]" type="hidden" value="' +
                    item.row.material_total_cost +
                ...
                tr_html +=
                    '<td class="text-right"><input class="form-control input-qty kb-pad text-center realuprice" name="real_unit_price[]" type="text" value="' +
                ...
                tr_html += item_material == null ?
                    '<td class="text-center"><span class="text-right material id="material_' +
                    row_no +
                    '">' +
                    '<i style="font-size:14px;">unselected</i>' +
                    '</span></td>' :'<td class="text-center"><span class="text-right material" id="material_' +
                    row_no +
                    '">' +
                    item_material +
                    '</span></td>';
                    ...
                    row_no +
                    '">' +
                    formatMoney((parseFloat(net_price) + parseFloat(pr_tax_val) + parseFloat(item_color_price) + parseFloat(item_upcharge_price) + parseFloat(item_material_price)) * parseFloat(item_qty)) +
                    '</span></td>';
            tr_html += '<td class="text-center"><i class="fa fa-trash-o tip pointer posdel" id="' + row_no + '" title="Remove"></i></td>';
                ...
                total += formatDecimal((
                    parseFloat(net_price) + 
                    parseFloat(pr_tax_val) + 
                    parseFloat(item_color_price) + 
                    parseFloat(item_upcharge_price) + 
                    parseFloat(item_material_price)) * parseFloat(item_qty), 4);
...
$(document).ready(function() {
    ...
    $('#payment').click(function() {
        if (count <= 1) {
            ...
            // console.log(spositems);                
            var itemsDetails = JSON.stringify(spositems);
    ...
    $('#edit-customer').click(function() {
        let edit_id=$('#spos_customer').append($('<option></option>')).val()
        if(!edit_id)alert("again!")
        else{
            
            $.ajax({
                type: 'get',
                url: base_url + 'pos/editCustomer',
                data: { id:edit_id },
                dataType: 'json',
                success: function(res) {
                      ...
                      $('#ecity').val(res.city);
                    //   console.log(res)
                    //  if($('#eNone').is(":checked"))  {data_starch = 'None';}
                    //   if(res.shirt_starch == 'None') {$('#eNone').prop("checked", true);}
                    //   else if (res.shirt_starch == "Light") {$('#eLight').prop("checked", true)}
                    //   else if(res.shirt_starch == "Medium") {$('#eMedium').prop('checked', true)}
                    //   else if(res.shirt_starch == "Extra") {$('#eExtra').prop('checked', true)}
                    //   else if(res.shirt_starch == "Heavy") {$('#eHeavy').prop("checked", true)}
                    //   $("input[name='shirt_starch'][value='"+res.shirt_starch+"']").prop('checked', true);
                           
                    $("input[name='shirt_starch'][value='"+ res.shirt_starch +"']").parent('.iradio_square-blue').addClass('checked');                  
                    $("input[name='packing'][value='"+ res.packing +"']").parent('.iradio_square-blue').addClass('checked');
                    $("input[name='kind_phone'][value='" + res.kind_phone + "']").parent('.iradio_square-blue').addClass('checked');

                    //   if(res.packing == "Hanger") {$('#eHanger').prop('checked', true)}
                    //   else if(res.packing == "Box") {$('#eBox').prop('checked', true)}
                    //   else if(res.packing == "HongLong") {$('#eHongLong').prop('checked', true)}
    ...
    $('#customer_edit').on('hidden.bs.modal', function(e) {
        $('#ename').val('');
        $('#eemail').val('');
        $('#ephone').val('');
        $('#ecf1').val('');
        $('#ecf2').val('');
        $("input[name='shirt_starch']").parent('.iradio_square-blue').removeClass('checked');                  
        $("input[name='packing']").parent('.iradio_square-blue').removeClass('checked');
        $("input[name='kind_phone']").parent('.iradio_square-blue').removeClass('checked');
    ...
    $('#update_customer').on('click', function(e) {              //submit
        e.preventDefault();
        let edit_id=$('#spos_customer').append($('<option></option>')).val();
        let data_name = $('#ename').val();
        let data_phone = $('#ephone').val();
        let data_address = $('#eaddress').val();
        let data_email = $('#eemail').val();
        let data_city = $('#ecity').val();
        let data_starch; // =  $("input[name='shirt_starch'][checked='checked']").val();
        
        let data_kind_phone;

        if ($('#ecellphone').is(":checked")) { data_kind_phone = 'cellphone'; }
        else if ($('#evoicephone').is(":checked")) { data_kind_phone = 'voicephone'; }
    
        // $("input[name='shirt_starch'][value='"+res.shirt_starch+"']").prop('checked', true)
        if($('#eNone').is(":checked"))  {data_starch = 'None';}
        else if($('#eLight').is(":checked")) {data_starch = 'Light';}
        else if($('#eMedium').is(":checked")) { data_starch = 'Medium';}
        else if($('#eHeavy').is(":checked"))  {data_starch = 'Heavy';}
        else if($('#eExtra').is(":checked"))  {data_starch = 'Extra';}
      
        let data_packing; // = $("input[name='packing'][checked='checked']").val();
       
        if($('#eHanger').is(":checked"))  {data_packing = 'Hanger'; }
        else if($('#eBox').is(":checked")) {data_packing = 'Box';}
        else if($('#eHangLong').is(":checked")) { data_packing = 'HANGLONG';}
        
        let data_cf1 = $('#ecf1').val();
        let data_cf2 = $('#ecf2').val();
        $.ajax({
            type: 'get',
            url: base_url + 'pos/updateCustomer',
            data: { 
                id:edit_id, 
                name:data_name, 
                phone:data_phone, 
                ...
                email:data_email, 
                city:data_city, 
                shirt_starch:data_starch,
                packing:data_packing,
                cf1:data_cf1,
                cf2:data_cf2,
                kind_phone: data_kind_phone,            
            },//$(this).serialize()},
            dataType: 'json',
            success: function(res) { 
...
function formatRepo (repo) {
    ...
    var $container = $(
        "<div style='display:flex; justify-content:space-between;'><span>"+repo.text + "</span><span>(" + repo.phone+ ")</span></div>"
    );
    return $container;
}
...
$(document).ready(function($) {
    ...
    if (sid && (spos_customer = get('spos_customer'))) {
        ...
        $('#spos_customer').select2({
            ajax: {
            url: function (params) {
                if(params.term==undefined) return base_url + 'pos/searchCustomer?search=';
                else return base_url + 'pos/searchCustomer?search=' + params.term;
            },
            dataType: 'json',
            delay: 0,
    ...
    $('#spos_customer').change(function() {
        ...
        $.ajax({
            ...
            success: function(res) {
				$('#pos_modal').show();
                $('#note_cf1').text(res.cf1);
                $('#note_cf2').text(res.cf2);
        ...
        $('.edit_customer').removeClass('hide');
    });
});
...
$(document).on('click', '.color_boxes', function() {
    ...
    if ($('#posTable tbody tr.selected')) {
        ...
        spositems[selected_id]['row'].color_name = color;
        spositems[selected_id]['row'].color_total_cost = color_price;
        store('spositems', JSON.stringify(spositems));
        // console.log(spositems[selected_id]);
        loadItems();
...
$(document).on('click', '.upcharge_boxes', function() {
    ...
    if ($('#posTable tbody tr.selected').length > 0) {
        selected_id = $('#posTable tbody tr.selected').attr('data-item-id');
        if(spositems[selected_id]['row'].upcharge_name == null) spositems[selected_id]['row'].upcharge_name = upcharge_name;
        else spositems[selected_id]['row'].upcharge_name += ', ' + upcharge_name;
        spositems[selected_id]['row'].upchrges_total_cost = parseFloat(spositems[selected_id]['row'].upchrges_total_cost) + parseFloat(upchrges_price);
        store('spositems', JSON.stringify(spositems));
        // console.log(spositems[selected_id]);
        loadItems();
...
$(document).on('click', '.material_boxes', function() {
    ...
    if ($('#posTable tbody tr.selected').length > 0) {
        selected_id = $('#posTable tbody tr.selected').attr('data-item-id');
        if ($('#posTable tbody tr.selected')) {
            if (spositems[selected_id]['row'].material_name == null) {
                spositems[selected_id]['row'].material_name = material_name;
            }else {
                spositems[selected_id]['row'].material_name += ', ' + material_name;
            }
            spositems[selected_id]['row'].material_total_cost = parseFloat(spositems[selected_id]['row'].material_total_cost) + parseFloat(material_price);
            // selected_id = $('#posTable tbody tr.selected').attr('data-item-id');
            // spositems[selected_id]['row'].upcharge_name = upcharge_name;
            // spositems[selected_id]['row'].upchrges_total_cost = upchrges_price;
            store('spositems', JSON.stringify(spositems));
            loadItems();
            // $('#posTable tbody tr.selected').find('.uupcharge').html(upcharge_name);
            $('#color-list').removeClass("control-bottombar-open");
