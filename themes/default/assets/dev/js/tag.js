var selected_id = null, current_staff = null;
...
function add_invoice_item(item) {
    ...
    if (receiptitems[item_id]) {
        receiptitems[item_id].row.qty = parseFloat(receiptitems[item_id].row.qty) + 1;
    } else {
        receiptitems[item_id] = item;
    }
    // console.log(receiptitems);

    store('receiptitems', JSON.stringify(receiptitems));
    loadItems();
    return true;
}
...
function loadItems() {
    ...
    if (get('receiptitems')) {
        ...
        // console.log(get('receiptitems'));
        receiptitems = JSON.parse(get('receiptitems'));
        // console.log('receiptitems =>', receiptitems)
        if(receiptitems) {
            $.each(receiptitems, function() {
                var item = this;
                var item_id = Settings.item_addition == 1 ? item.item_id : item.id;
                receiptitems[item_id] = item;
                ...
                var product_id = item.row.id,
                    ...
                    item_name = item.row.product_name.replace(/"/g, '&#034;').replace(/'/g, '&#039;'),
                    item_color = item.row.color_name,
                    item_color_price = item.row.color_cost,
                    item_upcharge = item.row.upcharge_name,
                // change upcharge price (current only press. etc.....)    myone
                    item_upcharge_price = item.row.upcharge_name == 'Press Only'? 0: item.row.upcharge_cost;
                    item_set_pc = item.row.set_pc;
                    item_press_only = item.row.press_only;
                // change price to upcharge price(current only press. etc.....)    myone
                var virtual_unit_price = item.row.upcharge_name == 'Press Only'? item.row.press_only : item.row.real_unit_price;

                var unit_price = parseFloat(virtual_unit_price);
                ...
                tr_html =
                    ...
                    item.row.color_name +
                    '"><input name="color_cost[]" type="hidden" value="' +
                    item.row.color_total_cost +
                    '"><input name="upcharge_name[]" type="hidden" value="' +
                    item.row.upcharge_name +
                    '"><input name="upcharge_cost[]" type="hidden" value="' +
                    item.row.upchrges_total_cost +
                    '"><input name="set_pc[]" type="hidden" value="' +
                    item.row.set_pc +
                    '"><button type="button" class="btn bg-purple btn-block btn-xs edit w-auto min-h-40" id="' +
                    item.row.id +
                    '" data-item="' +
                    item_id +
                    ...
                    item_code +
                    ')</span></button></td>';
                // <input class="rprice" name="net_price[]" type="hidden" id="price_' + item.row.id + '" value="' + formatDecimal(item_price) + '">
                tr_html +=
                    '<td class="text-right"><input class="form-control input-qty kb-pad text-center realuprice" name="real_unit_price[]" disabled type="text" value="' +
                    formatDecimal(virtual_unit_price) +
                    '" onClick="this.select();"><input class="rdiscount" name="product_discount[]" type="hidden" id="discount_' +
                    item.row.id +
                    '" value="' +
                    ds +
                    '"><span class="text-right sprice hide" id="sprice_' +
                    item.row.id +
                    '">' +
                ...
                tr_html +=
                    ...
                    item_was_ordered +
                    '"><input class="form-control input-qty kb-pad text-center rquantity" disabled name="quantity[]" type="text" value="' +
                ...
                tr_html += item_color == null ?
                    '<td class="text-center"><span class="text-right ccolor" id="ccolor_' +
                    item.row.id +
                    '">' +
                    '<i style="font-size:14px;">unselected</i>' +
                    '</span></td>' : item_color !== 'Skip' ? '<td class="text-center"><span class="text-right ccolor" id="ccolor_' +
                    item.row.id +
                    '">' +
                    '<i class="fa fa-circle" style="color: ' + item_color + ';"></i>' : '<td class="text-center"><span class="text-right ccolor" id="ccolor_' +
                    item.row.id +
                    '">' +
                    '' +
                    '</span></td>';
                tr_html += item_upcharge == null ?
                    '<td class="text-center"><span class="text-right uupcharge" id="uupcharge_' +
                    item.row.id +
                    '">' +
                    '<i style="font-size:14px;">unselected</i>' +
                    '</span></td>' :'<td class="text-center"><span class="text-right uupcharge" id="uupcharge_' +
                    item.row.id +
                    '">' +
                    item_upcharge +
                    '</span></td>';
                // console.log(net_price, pr_tax_val, item_color_price, item_upcharge_price);
                tr_html +=
                    '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' +
                    item.row.id +
                    '">' +
                    formatMoney((parseFloat(net_price) + parseFloat(pr_tax_val) + parseFloat(item_color_price) + parseFloat(item_upcharge_price)) * parseFloat(item_qty)) +
                    '</span></td>';
                tr_html += '<td class="text-center"><input class="tip pointer posdel_" type="checkbox" id="re_' + item.row.id + '" title="Reorder" checked/ ></td><td class="text-center"><i class="fa fa-level-up tip pointer posdel" id="' + item.row.id + '" title="Remove"></i></td>';
                newTr.html(tr_html);
                newTr.prependTo('#posTable');
                $('#posTable tbody tr').removeClass("selected");
                if (selected_id == null) $('#posTable tbody tr:first-child').addClass("selected");
                else $('#posTable tbody tr.' + selected_id).addClass("selected");
                $('#metrial-list').removeClass("control-bottombar-open");
                $('#damage-list').removeClass("control-bottombar-open");
                $('#color-list').removeClass("control-bottombar-open");
                total += formatDecimal((parseFloat(net_price) + parseFloat(pr_tax_val) + parseFloat(item_color_price) + parseFloat(item_upcharge_price)) * parseFloat(item_qty), 4);
                count += parseFloat(item_qty) * parseFloat(item_set_pc);
                an++;
...
$(document).ready(function() {
    ...
    $(document).on('click', '#editItem', function() {
        ...
        (receiptitems[item_id].row.qty = parseFloat($('#nQuantity').val())),
        (receiptitems[item_id].row.real_unit_price = price),
        (receiptitems[item_id].row.comment = $('#nComment').val()),
        (receiptitems[item_id].row.discount = $('#nDiscount').val() ? $('#nDiscount').val() : '0'),
    ...
    Row quantity change
    ============================= */
    $(document).on('keyup', '.rquantity', function() { // change
        var row = $(this).closest('tr');
        ...
        receiptitems[item_id].row.qty = new_qty;
        localStorage.setItem('receiptitems', JSON.stringify(receiptitems));
        loadItems();
    });
    ...
    $(document).on('change', '.realuprice', function() {
        ...
        var new_price = parseFloat($(this).val()),
            item_id = row.attr('data-item-id');
        receiptitems[item_id].row.real_unit_price = new_price;
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data !== null) {
                    ...
                    $('#color-list').addClass("control-bottombar-open");
                    $('#metrial-list').removeClass("control-bottombar-open");
                    $('#damage-list').removeClass("control-bottombar-open");
                } else {
                ...
                buttons: {
                    ...
                    success: {
                        ...
                        callback: function() {
                            var pos_pin = md5($('#pos_pin').val());
                            if (pos_pin == Settings.pin_code) {
                                delete receiptitems[item_id];
                                row.remove();
                                if (receiptitems.hasOwnProperty(item_id)) {} else {
                                    localStorage.setItem('receiptitems', JSON.stringify(receiptitems));
                                    loadItems();
                                }
                            } else {
    ...
    $('#payment').click(function() {
        if (count <= 1) {
            ...
            // console.log(receiptitems);                
            var itemsDetails = JSON.stringify(receiptitems);
            // console.log(itemsDetails);
            $('#items_detail').val(itemsDetails);
            $('#item_count').text(parseInt(count - 1));
            $('#order_quantity').val(count - 1);
            $('#order_items').val(an - 1);
            $('#balance').text('0.00');            
            $('#paid_by_val').val('due');
            $('#pickup_date_payment').text($('#pickup_date').val());
    ...
    $("#amount").on('keyup', function (e) {
        if ($('#amount').val() >= parseFloat($('#twt').text())) {
            $('#paid_by').val('paid');
            $('#paid_by_val').val('paid');            
        } else {
            if ($('#amount').val() <= 0) {
                $('#paid_by').val('due');
                $('#paid_by_val').val('due'); 
    ...
    $('#payModal').on('change', '#paid_by', function() {
        ...
        if (p_val == 'due') {
            ...
            $('#amount').val(parseFloat($('#twt').text()));
            $('#amount_val').val($('#amount').val());
            var total_paying = $('.amount').val();
            $('#total_paying').text(formatMoney(total_paying));
            if (Settings.rounding != 0) {
        ...
        } else if (p_val == 'partial') {
            $('#amount').val(parseFloat($('#twt').text())/2);
            $('#amount_val').val($('#amount').val());
            var total_paying = $('.amount').val();
            $('#total_paying').text(formatMoney(total_paying));
            if (Settings.rounding != 0) {
...
function createWin(data) {
    return new Promise(function(resolve) {
        ...
        var mywindow = window.open(d, 'spos_print', 'height=700,width=970');
...
// check pin funtion myone
// function checkSecPin(e) {
...
function formatRepo (repo) {
    ...
    var $container = $(
        "<div style='display:flex; justify-content:space-between;'><span> Ticket Number: &nbsp;&nbsp;&nbsp;" + repo.id + "</span><span>"+repo.text + " (" + repo.phone+ ")" +"</span></div>"
    );
    return $container;
}
...
function formatState (repo) {
    ...
    var $state = $(
      "<div style='display:flex; justify-content:space-between;'><span> Ticket Number: &nbsp;&nbsp;&nbsp;" + repo.id + "</span><span style='margin-right: 10px;'>"+repo.text + " (" + repo.phone+ ")" +"</span></div>"
    );
...
$(document).ready(function($) {
    ...
    $('.spot-content').fadeOut();
    // myone from
    // init pin & select & datepicker
    // var sec_key = $('#hidden_sec_pin').val();
    ...
    // console.log(sid);
    if (sid && (spos_customer = get('spos_customer'))) {
        spos_customer_name = get('spos_customer_name');
        // console.log(spos_customer, get('spos_customer'));
        $('#spos_customer').html('<option value="'+spos_customer+'">'+spos_customer_name+'</option>');
        // $('#spos_customer').select2();
        $('select').val(spos_customer).trigger('change');
        // show pick up date
        $('.datepicker').focus();
    } else {
        $('#spos_customer').select2({
    ...
    $('#pickup').val($('#pickup_date').val());

    $('#confirmIcon').on('click', function() {
        // show pick up date
        $('.datepicker').focus();
    });

        let cameraView = '<table><tr><td style="width:100%; height:300px; vertical-align: top;">'
        +
        '<div style="width:100%; height:auto" id="add_image"></div>'
        +
        '</tr></td>'
        +
        '<tr><td>'
        ...
        '<div class="w-100">'
        +
        '<span style="float:left"><input type="button" class="btn btn-info text-lg" " id="photo_bt" value="Ready"></span>'
        +
        '<span style="float:left"><input type="button" class="btn btn-info text-lg" " id="image_bt" value="take" style="margin-left:200px;"></span>'
        +
        '</div>'
        +
        '</td></tr>'              
        ...
        + 
        '<input class="hidden" id ="pic_name" value=""/>'
        +
        '</td></tr></table>';
    ...
    $('#spos_customer').change(function() {        
        ...
        $.get( base_url+'tag/view?id='+receipt_id, function( res ) {
            ...
            if(res.data.rows) {
                $.each(res.data.rows, function(i, row) {
                    receiptitems.push({
                        ...
                        id: "15697444847048",
                        item_id: "2",
                        label: "Men Suits 3pc (003001)",
                        row: row
                    });
                    // tag
                    // if (parseInt(row.quantity)){}
                });
                
               
                // console.log('ha', receiptitems)
                store('receiptitems', JSON.stringify(receiptitems));
                loadItems();
                // $('#invoice-panel').attr('src', '/pos/view/'+receipt_id);
                // var iframe = document.getElementById("invoice-panel");
                // var timer = setInterval(function() {
                //     var wrapper = iframe.contentWindow.document.getElementById("wrapper");
                //     var elmnt = iframe.contentWindow.document.getElementById("buttons");
                //     if (wrapper && elmnt) {
                ...
                //     }
                // }, 200);
                // var cus_status = res.data.inv.status=='due'? 'balance due':res.data.inv.status=='paid'?'Full paid':'Partial';
                // $('#cus_status').text(cus_status);
                $('#cus_name').text(res.data.customer.name);
                // console.log(res.data.customer.id);

                // $('#cus_email').text(res.data.customer.email);
                // $('#cus_phone').text(res.data.customer.phone);
                // var shirt_starch = res.data.customer.shirt_starch?res.data.customer.shirt_starch: '';
                // var packing = res.data.customer.packing?res.data.customer.packing: '';
                // $('#cus_hanger').text(shirt_starch + ', ' + packing);
                // $('#total_amount').text(formatMoney(formatMoney(res.data.inv.grand_total) - formatMoney(res.data.inv.order_tax)));
                // $('#order_tax').text(formatMoney(res.data.inv.order_tax));
                // $('#grand_total').text(formatMoney(res.data.inv.grand_total));
                // $('#paid_amount').text(formatMoney(res.data.inv.paid));
                // $('#due_amount').text(formatMoney(formatMoney(res.data.inv.grand_total) - formatMoney(res.data.inv.paid)));
                // $('#pickup_dat').text(res.data.inv.pickup_date)
                $('#total_num').text(res.data.tags.length);
                var codeObj = JSON.parse(res.data.inv.items_detail);
                Object.keys(codeObj).map(function(i){
                    // console.log(i, codeObj[i].row.barcode_symbology)
                    $('#code').val(codeObj[i].row.barcode_symbology);
                });

                $('#target_date').val(res.data.inv.pickup_date.split('-')[1]+'/'+res.data.inv.pickup_date.split('-')[2]+'/'+res.data.inv.pickup_date.split('-')[0])
                $('#drop_dat').text(res.data.inv.date);
                // tags img...
                var tags_html = '';
              
                $.each(res.data.tags, function(i, tag) {
                    var tagName = 'C';
                    if(tag.lgID.indexOf('Extra Care') > -1 || tag.lgID.indexOf('Color dots') > -1 || tag.lgID.indexOf('Repair') > -1 || tag.lgID.indexOf('Starch') > -1 || tag.lgID.indexOf('Ink') > -1 || tag.lgID.indexOf('Stains') > -1 || tag.lgID.indexOf('Ink') > -1){   
                        tagName = 'R';
                    } else if(tag.lgID.indexOf('Press Only') > -1) {
                        tagName = 'P';
                    }                  
                    tags_html += '<div class = "bottom_line" ><div class="tag-item"><div class="d-flex justify-content-between"><h5 class="my-0" style="margin-left: 1rem;"><b>'+tagName+res.data.invoice_id.replace('#', '')+'</b></h5><h5 class="my-0 text-right" style="margin-right: 1rem;">'+ (i+1) +' of '+ res.data.tags.length +'</h5></div>';
                    tags_html += '<div class="d-flex justify-content-between">'+tag.barcode+'<div  style="margin-right: 1rem; text-align: right;"><h5 class="text-right my-0 customer-name" style="margin-bottom: 5px!important;">'+ res.data.customer.name +'</h5><h5 class="my-0">'+ res.data.customer.phone +'</h5><h5 class="my-0 due-date">Due: '+ res.data.inv.pickup_date.split('-')[1]+'/'+res.data.inv.pickup_date.split('-')[2] +'</h5><h5 class="date due-date my-0"></h5></div></div></div></div>'; 
                });
                $('#print-section').html(tags_html);  
                $('#print-picture').html(cameraView);
                var test = res.data.image;
                var pic_id = test.id;
                var pic_count = test.total_quantity;

                if(test.snaps){ 
                    var pic_name = test.snaps.split(',');               
                    for(var d=1; d<=pic_count; d++){
                        let picture = '<div  style="width: 40%; float: left; margin: 10px;"><img style="width:100%; height:100px; float: left;" src="uploads/snaps/'+ pic_name[d] + '" alt="" class="pic_'
                        + pic_id + '_' + d +
                        '"></div>';    
                           
                        $('#add_image').prepend(picture);
                ...
                $('#image_bt').click( function(){   
                    //    if(!Webcam.attach()) return false;
                    if(flag === 1) {
                        a += 1; 
                        if(res.data.tags.length>=a){                      
                            Webcam.snap(function(data_uri) {             
                                images_html = '<img id="imageprev" style=" width:40%; height:100px; float: left; margin: 10px" src="'+ data_uri +'"/>';
                                $('#add_image').prepend(images_html);
                            });   
                    
                            var base64image = document.getElementById("imageprev").src;       
                            var image_id = receipt_id;
                            var tag_length = a;
                            var uri = base_url+'tag/checkImage?image_id=' + receipt_id + '&tag_length=' + a ;
                            Webcam.upload(base64image, uri, function(code, text){   
                                return text;                
                            });                                                   
                                return a;  
                        }else{  
                            alert('how math categories are there?!');
                            return false;                        
                        } 
                    } else return false
                });   
...
$('#tag_print').click(function(){
    var receipt_id = $('#spos_customer').val();    
    var tdate = new Date();
    ...
    var time1 = hh + ":" + mm + ":" + tdate.getSeconds()
    var currentDate=( MM+1)+ "/" + dd + '/' + yy +'&nbsp;'+ time ;
    var currentDate_year = yy + "-" + (MM+1) + "-" + dd + ' ' + time1;
    var tags_d_html = '';
    tags_d_html += 'Date Printed:<br/>' +'' +currentDate;
    $('#print-section .date').html(tags_d_html); 
    
    $('#print_bt').on('click', function () {
        $.get(base_url+'tag/printDate', {print_date:currentDate_year, print_id:receipt_id}, function(e){
            // console.log(e);
            $('#print_modal').hide();
        });
    })
...
$(document).on('click', '#lock_staff', function() {
    sessionStorage.clear();
});

$(document).on('keyup', '.select2-search__field', function(e) {
    $('#customerModal input[name="name"]').val($(this).val());
});
...
$(document).on('click', '#metrial', function() {
    // $('#categories-list').removeClass("control-sidebar-open");
    $('#color-list').removeClass("control-bottombar-open");
    $('#damage-list').removeClass("control-bottombar-open");
    $('#metrial-list').addClass("control-bottombar-open");
});
$(document).on('click', '#damage', function() {
    // $('#categories-list').removeClass("control-sidebar-open");
    $('#color-list').removeClass("control-bottombar-open");
    $('#metrial-list').removeClass("control-bottombar-open");
    $('#damage-list').addClass("control-bottombar-open");
});

$(document).on('click', '#posTable tbody tr', function() {
    $('#posTable tbody tr').removeClass('selected');
    $(this).addClass('selected');
    $('#metrial-list').removeClass("control-bottombar-open");
    $('#damage-list').removeClass("control-bottombar-open");
...
$('.datepicker').on('blur', function() {
    // if($('.datepicker.datepicker-dropdown')) {        
        // console.log('just picked date');
    // } else {
        $('#metrial-list').removeClass("control-bottombar-open");
        $('#damage-list').removeClass("control-bottombar-open");
        $('#color-list').removeClass("control-bottombar-open");
...
$(document).on('click', '.color_boxes', function() {
    $('.color_boxes').removeClass('selected');
    $(this).addClass('selected');
    var color = $(this).attr('data-name');
    var color_price = $(this).attr('data-price');
    if ($('#posTable tbody tr.selected')) {
        // console.log(receiptitems);
        selected_id = $('#posTable tbody tr.selected').attr('data-item-id');
        receiptitems[selected_id]['row'].color_name = color;
        receiptitems[selected_id]['row'].color_total_cost = color_price;
        store('receiptitems', JSON.stringify(receiptitems));
        // console.log(receiptitems[selected_id]);
        loadItems();
        // $('#posTable tbody tr.selected').find('.ccolor').html('<i class="fa fa-circle" style="color: ' + color + ';"></i>');
        $('#color-list').removeClass("control-bottombar-open");
        $('#metrial-list').removeClass("control-bottombar-open");
        $('#damage-list').addClass("control-bottombar-open");
    } else alert('Please select Product');
});
$(document).on('click', '.spotlist_boxes', function() {
    $('.spot-content').fadeIn();
});
$(document).on('click', '.upcharge_boxes', function() {
    $('.upcharge_boxes').removeClass('selected');
    $(this).addClass('selected');
    var upcharge_name = $(this).attr('data-upcharge_name');
    var upchrges_price = $(this).attr('data-price');
    if ($('#posTable tbody tr.selected').length > 0) {
        selected_id = $('#posTable tbody tr.selected').attr('data-item-id');
        if(receiptitems[selected_id]['row'].upcharge_name == null) receiptitems[selected_id]['row'].upcharge_name = upcharge_name;
        else receiptitems[selected_id]['row'].upcharge_name += ', ' + upcharge_name;
        receiptitems[selected_id]['row'].upchrges_total_cost = parseFloat(receiptitems[selected_id]['row'].upchrges_total_cost) + parseFloat(upchrges_price);
        store('receiptitems', JSON.stringify(receiptitems));
        // console.log(receiptitems[selected_id]);
        loadItems();
        // $('#posTable tbody tr.selected').find('.uupcharge').html(upcharge_name);
        $('#color-list').removeClass("control-bottombar-open");
        $('#damage-list').removeClass("control-bottombar-open");
        $('#metrial-list').addClass("control-bottombar-open");
    } else {
...
$(document).on('click', '.material_boxes', function() {
    $('.material_boxes').removeClass('selected');
    $(this).addClass('selected');
    var material_name = $(this).attr('data-material_name');
    var material_price = $(this).attr('data-price');
    if ($('#posTable tbody tr.selected')) {
        // selected_id = $('#posTable tbody tr.selected').attr('data-item-id');
        // receiptitems[selected_id]['row'].upcharge_name = upcharge_name;
        // receiptitems[selected_id]['row'].upchrges_total_cost = upchrges_price;
        // store('receiptitems', JSON.stringify(receiptitems));
        // loadItems();
        // $('#posTable tbody tr.selected').find('.uupcharge').html(upcharge_name);
        $('#color-list').removeClass("control-bottombar-open");
...
function posScreen() {
    ...
    var items_dh = wh - 120,
        list_table_dh = wh - 185 - left_top_dh - total_dh - buttons_dh + 60;
